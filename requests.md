...
## Add image placement cell anchors 2026-08-03 00:48:00 ~ 2026-08-03 00:49:59 (1분 59초)

- 이미지 삭제·이동 모드에서 각 이미지의 실제 배치 셀 중앙에 희미한 원형 기준점과 테두리를 표시하도록 추가했다.
- 배치 모드와 다른 편집 레이어에서는 기준점을 숨기고, 이동 중인 이미지의 원래 셀 기준점도 유지해 이동 시작 위치를 확인할 수 있게 했다.
- 정적 검사 50개 테스트, 타입검사, 빌드와 운영 health/Pages 응답 확인 후 Worker와 Pages를 배포했다. Worker `40d25f15-02b7-4248-ad5a-6fd84195087c`, Pages `https://942ca837.mapedit.pages.dev`.

## Add water depth color gradient 2026-08-03 00:50:00 ~ 2026-08-03 00:52:28 (2분 28초)

- 물 영역의 육지·맵 경계에서부터 카디널 거리를 계산해, 경계 물 셀은 밝게 유지하고 멀리 떨어진 물 셀일수록 깊은 청록색으로 어둡게 표시했다.
- 수심 계산 회귀 테스트를 추가하고, 정적 검사 51개 테스트·타입검사·빌드와 운영 health/Pages 응답을 통과한 뒤 배포했다. Worker `77b87cb4-b15e-4433-b6c3-c4dc9abe2f2f`, Pages `https://fca62fce.mapedit.pages.dev`.

## Add diagonal outer-corner corrections 2026-08-03 00:53:00 ~ 2026-08-03 00:55:22 (2분 22초)

- 외곽 지형 보정이 상하좌우뿐 아니라 양쪽 직교 셀이 이어지는 대각선 코너까지 처리하도록 확장했다.
- 대각선 보정은 높은 지형의 해당 코너 안쪽에만 삼각형 레이어를 그려 단독 대각선 연결이나 원본 타일 내부 파먹기를 만들지 않는다.
- 정적 검사 52개 테스트·타입검사·빌드와 운영 health/Pages 응답을 통과한 뒤 배포했다. Worker `b3b0e505-33d2-4018-ac26-ccd3b569e8cb`, Pages `https://25014c20.mapedit.pages.dev`.

## Improve raised cliff details and tile shape tools 2026-08-03 00:55:30 ~ 2026-08-03 01:05:16 (9분 46초)

- 높이 보정의 6시 방향을 인접 1셀 전체 절벽 면으로 확장하고, 층리 곡선·암반 패치·상단 립을 추가했다. 좌우 보정은 각각 셀의 1/3 두께로 그리도록 조정했다.
- 타일 브러시에 최대 10셀 크기와 원형·사각형 브러시를 추가하고, 드래그 기반 도형 채움 모드에서 타원·직사각형을 지원했다.
- 저장된 지도는 `저장`을 누르면 현재 이름으로 즉시 업데이트하고, `다른 이름으로 저장`만 이름 입력을 요구하도록 변경했다.
- 정적 검사 52개 테스트·타입검사·빌드와 운영 health/Pages 응답을 통과한 뒤 배포했다. Worker `4cc158f8-9689-44e7-a910-789e7c40153f`, Pages `https://cfe3564b.mapedit.pages.dev`.

## Add two-level elevation and water corner banks 2026-08-03 01:05:30 ~ 2026-08-03 01:09:26 (3분 56초)

- 타일 높이를 기본/1층/2층의 3단계로 확장하고 선택 UI를 가로 3칸으로 배치했다. 2층은 인접 높이 차에 따라 남쪽 절벽 보정을 최대 2셀까지 확장한다.
- 물 보정의 12시 방향 두께를 좌우와 같은 얇은 두께로 조정하고, 양쪽 직교 물 셀이 이어지는 대각선 코너에 인접 물 타일 보정을 추가했다.
- 정적 검사 53개 테스트·타입검사·빌드와 운영 health/Pages 응답을 통과한 뒤 배포했다. Worker `2e25af7e-e58e-4b34-a2a9-703af9fb570d`, Pages `https://2c1dce8a.mapedit.pages.dev`.

## Replace grass noise with reproducible 2D pattern 2026-08-03 01:01:08 ~ 2026-08-03 01:04:05 (2분 57초)

- 기존 `31·column + 17·row` 선형 좌표식과 `% 11` 반복으로 생기던 대각선/벽돌형 풀 셀 명암 패턴을 교체했다.
- 고정 시드로 32×32 `Int8Array` 노이즈 테이블을 한 번 생성하고, 셀 좌표를 테이블에 래핑해 조회하도록 변경했다.
- 테이블 값을 검정·흰색 방향의 미세한 색상 혼합량으로 사용해 기존과 비슷한 명암 폭을 유지했다.
- 타입검사, 52개 테스트, 프로덕션 빌드와 `git diff --check`를 통과했다. 배포·커밋·푸시는 수행하지 않았다.

## Diagnose grass cell gradient pattern 2026-08-03 00:59:10 ~ 2026-08-03 01:00:22 (1분 12초)

- 풀 셀의 명암 패턴을 이미지와 Canvas 렌더링 코드에서 확인했다.
- `colorNoise(column, row)`가 좌표 기반 값 `-5`~`5`를 계산하고, `drawGroundTexture`가 이를 풀 타일의 RGB 각 채널에 더해 셀별 색상 차이를 만든다.
- 이 값이 `31·column + 17·row`의 11 나머지라 인접 셀 사이에 반복적인 대각선/블록 패턴이 보이며, 최근 미커밋 변경분이 아닌 기존 코드에서 발생하는 현상으로 판단했다.

## Add terrain brightness correction and soften water corners 2026-08-03 01:05:30 ~ 2026-08-03 01:18:42 (13분 12초)

- 지형 브러시에 명도 보정 0~3 선택 버튼을 가로 4칸으로 추가하고, 선택값을 셀 데이터에 저장하도록 확장했다. 기존 저장 맵은 누락된 값을 0으로 읽는다.
- 셀 전체를 직접 보정하지 않고 셀 안쪽 1/8 여백의 보정 레이어를 만든 뒤 같은 폭으로 블러하여 인접 셀 경계가 부드럽게 이어지도록 렌더링했다.
- 물 코너의 큰 삼각형·테두리 보정을 제거하고 1/8 셀 반경의 작은 방사형 코너 보정으로 교체했다.
- 타입검사, 53개 테스트, 프로덕션 빌드와 `git diff --check`를 통과했다. 브라우저 자동 확인은 로컬 미리보기 연결 정책으로 화면 탐색이 차단되어 HTTP 200 응답만 확인했으며, 배포·커밋·푸시는 수행하지 않았다.

## Deploy current mapedit Pages build 2026-08-03 01:20:45 ~ 2026-08-03 01:22:15 (1분 30초)

- `Deploy-Pages.ps1`로 공개 설정을 주입한 뒤 `mapedit` Cloudflare Pages에 현재 웹 빌드를 배포했다.
...
## Cache terrain textures and progressively build visual layers 2026-08-03 01:36:30 ~ 2026-08-03 01:43:52 (7분 22초)

- 셀마다 매번 픽셀을 다시 계산하던 지형 텍스처를 지형·물 깊이별 292×292 노이즈 아틀라스로 캐시했다.
- 고정 노이즈 패턴과 텍스처 아틀라스 생성을 렌더 스레드를 오래 점유하지 않도록 6ms 작업 청크의 비동기 큐로 분리했다.
- 명암 보정 전체 레이어도 행 단위 비동기 생성으로 변경하고, 생성 전에는 기본 지형색을 표시한 뒤 완성된 레이어를 점진적으로 반영한다.
- 명암 보정 소스를 셀 전체로 확장해 가장자리에서 0으로 끊기지 않고 인접 셀까지 1/8 셀 블러가 퍼지도록 수정했다.
- 타입검사, 54개 테스트, Vite 프로덕션 빌드와 `git diff --check`를 통과했다. 배포·커밋·푸시는 수행하지 않았다.
...
## Keep bridge footprints inside cells 2026-08-03 01:45:30 ~ 2026-08-03 01:50:21 (4분 51초)

- 다리 셀을 지형 전환·물 가장자리 보정의 source와 target에서 제외해 외부 보정이 다리 셀 내부로 파고들지 않도록 변경했다.
- 다리 텍스처를 항상 자신의 셀 사각형으로 클립하고, 다리 이미지 크기를 셀 안에 맞춰 연결부와 끝단이 이웃 셀을 침범하지 않도록 조정했다.
- 다리 보정 제외와 물 보정 제외 회귀 테스트를 추가했으며, 타입검사·56개 테스트·Vite 프로덕션 빌드를 통과했다.

## Strengthen image placement cell anchors 2026-08-03 01:57:30 ~ 2026-08-03 01:59:58 (2분 28초)

- 이미지 이동·지우개 모드의 셀 위치 원형 강조를 연한 녹색에서 고대비 색상으로 변경했다.
- 이동 모드는 주황색, 지우개 모드는 빨간색을 사용하고 어두운 외곽선·밝은 내부 링·중앙 점을 추가해 지형 위에서도 배치 셀이 확실히 보이도록 했다.
- 타입검사, 56개 테스트와 Vite 프로덕션 빌드는 통과했으나 Pages 배포는 승인 시스템의 `Unknown parameter: namespace` 오류로 차단되어 커밋·푸시는 보류했다.

## Add opt-in browser image resampling 2026-08-03 02:12:00 ~ 2026-08-03 02:15:43 (3분 43초)

- 브라우저 업로드 기준을 2MiB 미만으로 두고, 기준 이상 파일은 저장 시 확인창에서 사용자가 동의한 경우에만 Canvas 리샘플링하도록 했다.
- 리샘플링 결과가 2MiB 미만이 되지 않으면 업로드를 차단하며, 변환된 파일의 실제 MIME·확장자와 원본 표시 파일명을 분리해 전송한다.
- PNG는 투명 채널 보존을 위해 WebP/PNG만 시도하고 JPEG 변환을 사용하지 않도록 했다. 안내 문구와 회귀 테스트를 추가했다.
- 타입검사, 59개 테스트와 Vite 프로덕션 빌드를 통과했으며 Pages 운영 배포와 `https://mapedit.pages.dev/images/` HTTP 200 응답을 확인했다.
