...
function createSmallMap(): MapDocument {
  ...
  map.columns = 8;
  map.rows = 8;
  map.cells = Array.from({ length: 64 }, () => ({ ground: "grass" as const, elevation: 0 as const, brightnessCorrection: 0 as const, prop: null }));
  map.cells[0] = { ground: "water", elevation: 0, brightnessCorrection: 0, prop: "boulder" };
  map.cells[7] = { ground: "stone", elevation: 0, brightnessCorrection: 0, prop: "pine-tree" };
  map.cells[56] = { ground: "dirt", elevation: 0, brightnessCorrection: 0, prop: "shrub" };
  map.cells[63] = { ground: "water", elevation: 0, brightnessCorrection: 0, prop: "footbridge" };
  map.images = [
...
describe("resizeMap", () => {
  it("preserves cells from the selected top-left anchor and fills new cells", () => {
    ...
    expect(resized.cells[0]).toEqual({ ground: "water", elevation: 0, brightnessCorrection: 0, prop: "boulder" });
    expect(resized.cells[7]).toEqual({ ground: "stone", elevation: 0, brightnessCorrection: 0, prop: "pine-tree" });
    expect(resized.cells[8]).toEqual({ ground: "grass", elevation: 0, brightnessCorrection: 0, prop: null });
    expect(resized.cells[7 * 10]).toEqual({ ground: "dirt", elevation: 0, brightnessCorrection: 0, prop: "shrub" });
    expect(resized.cells[7 * 10 + 7]).toEqual({ ground: "water", elevation: 0, brightnessCorrection: 0, prop: "footbridge" });
    expect(original.columns).toBe(8);
    expect(original.cells[0]).toEqual({ ground: "water", elevation: 0, brightnessCorrection: 0, prop: "boulder" });
  });
  ...
  it("keeps the existing map centered when expanding", () => {
    ...
    expect(resized.cells[1 * 10 + 1]).toEqual({ ground: "water", elevation: 0, brightnessCorrection: 0, prop: "boulder" });
    expect(resized.cells[1 * 10 + 8]).toEqual({ ground: "stone", elevation: 0, brightnessCorrection: 0, prop: "pine-tree" });
    expect(resized.cells[8 * 10 + 1]).toEqual({ ground: "dirt", elevation: 0, brightnessCorrection: 0, prop: "shrub" });
    expect(resized.cells[8 * 10 + 8]).toEqual({ ground: "water", elevation: 0, brightnessCorrection: 0, prop: "footbridge" });
